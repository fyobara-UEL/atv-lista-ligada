...
void inicializarLista(LISTA* l) {
  l->nroElem = 0;
  l->capacidade = MAX;  // Definir capacidade inicial
  l->A = (REGISTRO*) malloc(l->capacidade * sizeof(REGISTRO)); // Alocação dinâmica
} /* inicializarLista */
...
void resize(LISTA* l, int novaCapacidade) {
    ...
    printf("Capacidade da lista ajustada para %d.\n", l->capacidade);
}
...
int tamanhoEmBytes(LISTA* l) {
    return l->capacidade * sizeof(REGISTRO) + sizeof(LISTA); //Antes estava retornando apenas o tamanho do ponteiro (16 Bytes)
} /* tamanhoEmBytes */
...
bool excluirElemLista(LISTA* l, TIPOCHAVE ch) {
  ...
  l->nroElem--;
      // Verifica se a lista deve ser redimensionada
  if (l->nroElem <= l->capacidade / 4) {
    printf("========================================");
    printf("A lista está com ocupação igual ou inferior a 25%%...\n");
...
bool excluirElemListaOrd(LISTA* l, TIPOCHAVE ch) {
    int pos, j;
    pos = buscaBinaria(l, ch);  // Busca o elemento na lista
    if (pos == ERRO) return false;  // Não encontrou o elemento

    // Move os elementos após o removido para a esquerda
    for (j = pos; j < l->nroElem - 1; j++) {
        l->A[j] = l->A[j + 1];
    }
    l->nroElem--;  // Decrementa o número de elementos
...
bool inserirElemListaOrd(LISTA* l, REGISTRO reg) {
    // Verificar se a lista precisa de redimensionamento antes de inserir
    if (l->nroElem >= l->capacidade) {
        printf("\n========================================");
        printf("\nA lista está cheia.\n");
        resize(l, l->capacidade * 2);  // Dobra a capacidade da lista
        printf("========================================");
    }
    ...
    l->A[pos] = reg;  // Inserir o elemento na posição correta
    l->nroElem++;     // Atualizar o número de elementos
